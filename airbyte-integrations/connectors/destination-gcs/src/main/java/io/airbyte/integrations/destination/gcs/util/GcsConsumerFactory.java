...
public class GcsConsumerFactory {

    private static final Logger LOGGER = LoggerFactory.getLogger(GcsConsumerFactory.class);
    ...
    private static Function<ConfiguredAirbyteStream, WriteConfig> toWriteConfig(final BlobStorageOperations storageOperations,
        ...
        return stream -> {
            ...
            final String fullOutputPath = storageOperations.getBucketObjectPath(namespace, streamName, SYNC_DATETIME, customOutputFormat.replaceAll("//${NAMESPACE}/${STREAM_NAME}/", "/"));
            ...
            for (final WriteConfig writeConfig : writeConfigs) {
                if (writeConfig.getSyncMode().equals(DestinationSyncMode.OVERWRITE)) {
                    ...
                    LOGGER.info("Clearing storage area in destination started for namespace {} stream {} bucketObject {} pathFormat {}",
                            namespace, stream, outputBucketPath, pathFormat.replaceAll("//${NAMESPACE}/${STREAM_NAME}/", "/"));
                    storageOperations.cleanUpBucketObject(namespace, stream, outputBucketPath, pathFormat.replaceAll("//${NAMESPACE}/${STREAM_NAME}/", "/"));
        ...
        return (pair, writer) -> {
            ...
            try (writer) {
                ...
                writeConfig.addStoredFile(storageOperations.uploadRecordsToBucket(
                        ...
                        writeConfig.getFullOutputPath().replaceAll("//${NAMESPACE}/${STREAM_NAME}/", "/")));
    ...
    private OnCloseFunction onCloseFunction(final BlobStorageOperations storageOperations,
        ...
        return (hasFailed) -> {
            if (hasFailed) {
                ...
                for (final WriteConfig writeConfig : writeConfigs) {
                    storageOperations.cleanUpBucketObject(writeConfig.getFullOutputPath().replaceAll("//${NAMESPACE}/${STREAM_NAME}/", "/"), writeConfig.getStoredFiles());
